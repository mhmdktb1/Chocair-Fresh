...
export async function getProductRecommendations(productId, options = {}) {
  ...
  if (!associations || Object.keys(associations).length === 0) {
    // No associations found, try similar products (same category) first
    // If that fails or returns too few, it will fall back to popular
...
/**
 * Get similar products based on category and popularity
 * 
 * @param {string} productId - Source product ID
 * @param {Object} options - Options
 * @returns {Promise<Array>} Array of similar products
 */
export async function getSimilarProducts(productId, options = {}) {
  const { limit = 10, excludeIds = [] } = options;

  if (!productCategories || !productPopularity) {
    await loadKnowledge();
  }

  const sourceCategory = productCategories[productId];
  if (!sourceCategory) {
  ...
  for (const [pid, category] of Object.entries(productCategories)) {
    if (pid === productId) continue;
    if (excludeIds.includes(pid)) continue;

    // Check if category matches
    if (category === sourceCategory) {
      const popularity = productPopularity[pid] || 0;
      candidates.push({
        productId: pid,
        score: popularity, // Score is just popularity for now
        category: category,
        popularity: popularity,
        name: productNames[pid] || 'Unknown'
      });
    }
  }
